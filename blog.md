# Insecurity of secret key usage
...
example that if you are not certain of what you are doing, then you should 
not do it.
...
choice was that in the past lack of secure sources of randomness resulted in the security flaw
...
* `keygen` returns a key-pair $(sk, vk)$. First, it chooses $ sk\leftarrow\{0,1\}^256 $ uniformly
  at random. Next, let $ (h_0, h_1, \ldots, h_{2b - 1})\gets\hash(sk) $,
  and compute the signing key, $ sig\_key \gets 2^{b-2} + \sum_{3\leq i\leq b-3}2^i h_i $
  . Finally, compute $ vk \gets sig\_key \cdot G $, and return $ (sk, vk) $.
* `sign`$(sk, vk, m)$ takes as input a keypair $ (sk, vk) $ and a message $ m $, and returns a
  ...
  integer in $ \{0,1,\ldots, 2^{2b}-1\} $. Let $ R \gets k\cdot G $, and $ s \gets (k + H(R, A, M)\cdot
  sig\_key) \mod p $. Return $ sig \gets (R, s) $. 
* `verify`$(m, vk, sig)$ takes as input a message $ m $, a verification key $ vk $ and a signature
  $ sig $, and returns $ r\in\{\accept, \reject\} $ depending on whether the signature is valid or not. The algorithm
...
* `GenProof`$(sk, vk, m)$ takes as input a keypair $(sk, vk)$ and a message
  ...
  derive $sig\_key$. Let $H \gets H_{s2c}(vk, m)$. Let $\Gamma \gets sk\cdot H$. Let 
  $ k \gets H(h_b, \ldots, h_{2b-1}, H) $. Let $c \gets \hash  (H\parallelsep\Gamma\parallelsep k\cdot G \parallelsep k\cdot H)[..128]$. 
  Compute $s \gets (r + c\cdot sig\_key)\mod\order$. Finally, return the proof $proof \gets (\Gamma, c, s)$ and the randomness
  $output \gets \hash(\texttt{suite\_string}\parallelsep 0x03\parallelsep\cofvar\cdot\Gamma\parallelsep
  0x00)$.
* `verify`$(m, \vrfvk, \vrfproof)$ takes as input a message $m$, a verification key $vk$ and a vrf proof
  $proof$, and returns $output$ or $ \false $. It parses the proof as $(\Gamma, c, s) = proof$, and
  computes $H\gets H_{s2c}(vk, m)$. Let $U \gets s\cdot G - c\cdotvk$ and $V \gets s\cdot
  H - c\cdot\Gamma$. Compute the challenge $c'\gets\hash(H\parallelsep\Gamma\parallelsep U\parallelsep V)[..128]$.
  If $c'=c$, then return $output \gets  \hash(\texttt{suite\_string}\parallelsep
...
$ k \gets H(h_b, \ldots, h_{2b-1}, m) $. In VRF, the value is computed via $ k \gets H(h_b, \ldots, h_{2b-1}, H) $. 
However, the challenge is computed via $H(R, A, M)$ and $\hash
...
And if one runs the script, we can see that we successfully faked an ed25519 signature!
...
[problems]() in some libraries that had an incorrect API) was to combine determinism and secure
